...
%% ── Column geometry ─────────────────────────────────────────────────────────
%  Four columns, each 44 mm wide, 11 mm gaps → total span = 4×44 + 3×11 = 209 mm
%  Centre columns at x = -82.5, -27.5, +27.5, +82.5  (half-span = 104.5 mm)
\def\colA{-82.5mm}
\def\colB{-27.5mm}
\def\colC{ 27.5mm}
\def\colD{ 82.5mm}
\def\colW{44mm}          % box width
\def\codeW{42mm}         % code-box width (2 mm narrower for visual gap)

%% ── Row y-positions (anchored to header = 0) ────────────────────────────────
%  NOTE: macro names must contain only letters — no trailing digits.
\def\yHeader  {  0mm}
\def\yStage   {-18mm}
\def\yTtwo    {-34mm}   % top of Tier-2 code boxes
\def\yTfour   {-57mm}   % top of Tier-4 code boxes
\def\ySummary {-84mm}
\def\yLegend  {-96mm}
...
\tikzset{
  ...
  t2box/.style={
    ...
    align=left,
    font=\fontsize{6.8}{8.5}\selectfont\ttfamily
  },
  t4box/.style={
    rectangle, rounded corners=3pt,
    minimum width=\codeW, text width=\codeW,
  ...
  badge/.style={
    ...
    font=\fontsize{7.5}{9}\selectfont\bfseries\sffamily,
    text=badgecol
  },
  pipeArrow/.style={
...
%% ── Speed badges (between the two code rows) ─────────────────────────────────
\node[badge] at (\colA, \yTfour - 0.5mm) {\textbf{6–13}$\boldsymbol{\times}$};
\node[badge] at (\colB, \yTfour - 0.5mm) {\textbf{7–13}$\boldsymbol{\times}$};

%% ════════════════════════════════════════════════════════════════════════════
%%  TIER-4 CODE BOXES
%% ════════════════════════════════════════════════════════════════════════════
\node[t4box, anchor=north] (t4a) at (\colA, \yTfour + 11mm) {%
...
\node[t4box, anchor=north] (t4b) at (\colB, \yTfour + 11mm) {%
  {\color{t4green}\fontsize{6.2}{7.5}\selectfont\bfseries\sffamily COMPILED}\\[2pt]%
  traj, state =\\
  \quad jax.lax.scan(\\
...
\node[t4box, anchor=north] (t4c) at (\colC, \yTfour + 11mm) {%
  {\color{t4green}\fontsize{6.2}{7.5}\selectfont\bfseries\sffamily COMPILED}\\[2pt]%
  jax.lax.scan(\\
  \quad gae\_step,\\
  \quad (0, V\_last),\\
...
\node[t4box, anchor=north] (t4d) at (\colD, \yTfour + 11mm) {%
  {\color{t4green}\fontsize{6.2}{7.5}\selectfont\bfseries\sffamily COMPILED}\\[2pt]%
  for ep in range(K):\\
  \quad perm = jrandom\\
...
\begin{scope}[on background layer]
  ...
  \fill[t2red!6, rounded corners=4pt]
    (-96mm, \yTtwo + 1mm) rectangle (96mm, \yTfour + 10mm);
  % Tier-4 band
  \fill[t4green!6, rounded corners=4pt]
    (-96mm, \yTfour + 9mm) rectangle (96mm, \ySummary + 5mm);
\end{scope}
